...
class TodoEndpoints[F[_]: Sync](todoService: TodoService[F], authenticationService: JwtAuthenticationService[F])
  ...
  val deleteTaskById =
    todoRoot
      ...
      .serverLogic {
        case (_: User, id: String) =>
          todoService.removeTask(id).map(Right[ErrorResponse, String])
      }
  ...
  private def taskOptionToResponse(task: Option[Existing]): Either[ErrorResponse, RetrieveTaskResponse] =
